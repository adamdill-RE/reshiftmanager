...
> **Received, August 2026 — description, corrections, then the committee's sketch, which fixed the compass.** North up: the tent (roughly 800 ft by 150 ft) runs north–south. The seven stops run up the tent's length from the Bus Ops complex at the south end — Reed nearest the office, then OST, Special Events & Woodlands, West Loop, Monroe, Maxey, Gold Badge / LT at the north end — Reed and OST the largest, the lone bathroom against the tent's north end. Guests come in on the west side through the Overheads / Tent Entrance lines, line up in the outside queue areas along the west wall, and enter the tent through the back gates — each stop's gate in from those queues. The seven bus lanes run along the east side: starters in the lanes even with their stop, the Bus Callers in the lanes at Special Events, computers/counters/runners between the tent's east wall and the lanes, where boarding happens. The Bus Ops office, log cabin, restrooms and chuck wagon sit against the south end with the committee gate beyond them. Holly Hall's gate lies beyond the drawing to the northeast (Curve drawn with it, exact spot unconfirmed), Naomi's — with the walkover bridge — to the southeast. `bin/gen-tarmac-map.php` generates `public/assets/map/tarmac.svg` from all of this; the Unload cluster's placement is likewise unconfirmed. The requirements below stay as the contract any future replacement drawing must meet.
