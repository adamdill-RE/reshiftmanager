...
> **Received, August 2026, corrections folded in.** Rodeo Express supplied the layout as a written description — tent roughly 800 ft by 150 ft, seven bus lanes west, seven stop bands north to south (Reed, OST, Special Events & Woodlands, West Loop, Monroe, Maxey, Gold Badge / LT at the south end), back gates just outside the east wall, support crew just outside the west wall, starters in the lanes even with their stops, the Bus Callers in the lanes at Special Events, overheads at the southeast entrance, the Bus Ops complex against the tent's north wall with the committee gate on its north side, Naomi and Curve/Holly Hall beyond the drawing. `bin/gen-tarmac-map.php` generates `public/assets/map/tarmac.svg` from it; the one placement still unconfirmed is the Unload cluster, revised like any other by editing the generator and rerunning it. The requirements below stay as the contract any future replacement drawing must meet.
