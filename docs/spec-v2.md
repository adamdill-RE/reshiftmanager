...
> **Received, August 2026 — description, corrections, then the committee's sketch, which fixed the compass.** North up: the tent (roughly 800 ft by 150 ft) runs north–south. Guests enter through the Overheads / Tent Entrance lines at the north end and walk south to their stop; the seven stops run down the tent's length from the entrance — Reed, OST, Special Events & Woodlands, West Loop, Monroe, Maxey, Gold Badge / LT at the south end — Reed and OST the largest. The seven bus lanes run along the east side: starters in the lanes even with their stop, the Bus Callers in the lanes at Special Events, computers/counters/runners between the tent's east wall and the lanes. The west side is the back of the tent, back gates just outside it. The Bus Ops office, log cabin, restrooms and chuck wagon sit against the south end with the committee gate beyond them; a lone bathroom against the north end. Holly Hall's gate lies beyond the drawing to the northeast (Curve drawn with it, exact spot unconfirmed), Naomi's — with the walkover bridge — to the southeast. `bin/gen-tarmac-map.php` generates `public/assets/map/tarmac.svg` from all of this; the Unload cluster's placement is likewise unconfirmed and flagged. The requirements below stay as the contract any future replacement drawing must meet.
